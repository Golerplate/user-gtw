...
func Test_UpdateUsername(t *testing.T) {
	t.Run("ok - username", func(t *testing.T) {
		...
		// setup body
		body := &handlers_http_private_current_user_v1.UpdateUsernameRequest{
			Username: "testuser_refreshed",
		}
		...
		// setup http handler
		e := echo.New()
		req := httptest.NewRequest(http.MethodPut, "/private/v1/current-user/username", strings.NewReader(string(reqBody)))
		...
		data := handlers_http_private_current_user_v1.UpdateUsernameResponse{
		...
		// test
		if assert.NoError(t, handler.UpdateUsername(c)) {
	...
	t.Run("nok - bind fail", func(t *testing.T) {
		...
		// setup http handler
		e := echo.New()
		req := httptest.NewRequest(http.MethodPut, "/private/v1/current-user/username", strings.NewReader(string(body)))
		req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
		rec := httptest.NewRecorder()
		c := e.NewContext(req, rec)
		...
		formattedResponse := pkghttp.NewHTTPResponse(http.StatusBadRequest, pkghttp.MessageBindError, nil)
		resp, err := json.Marshal(formattedResponse)
		assert.NoError(t, err)
	...
	t.Run("nok - client fail", func(t *testing.T) {
		...
		m.EXPECT().UpdateUsername(gomock.Any(), userid, "testuser_refreshed").Return(nil, pkgerrors.NewInternalServerError("client error"))
	...
	t.Run("nok - cuser missing", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		m := user_store_svc_v1_mocks.NewMockUserStoreSvc(ctrl)
